...
function renderRichHTML(text) {
  ...
  for (const raw of lines) {
    ...
    // Headings
    if (/^####\s/.test(line)) { closeList(); closeBlockquote(); out.push(`<h4 class="text-sm font-semibold text-stone-800 mt-5 mb-1.5">${inlineFmt(line.replace(/^####\s*/, ''))}</h4>`); continue; }
    if (/^###\s/.test(line)) { closeList(); closeBlockquote(); out.push(`<h3 class="text-base font-semibold text-stone-900 mt-6 mb-2">${inlineFmt(line.replace(/^###\s*/, ''))}</h3>`); continue; }
    if (/^##\s/.test(line)) { closeList(); closeBlockquote(); out.push(`<h2 class="text-lg font-semibold text-stone-900 mt-7 mb-2">${inlineFmt(line.replace(/^##\s*/, ''))}</h2>`); continue; }
    if (/^#\s/.test(line)) { closeList(); closeBlockquote(); out.push(`<h2 class="text-xl font-bold text-stone-900 mt-8 mb-3">${inlineFmt(line.replace(/^#\s*/, ''))}</h2>`); continue; }

    // Horizontal rule
    if (/^---+$/.test(line.trim())) { closeList(); closeBlockquote(); out.push('<hr class="my-6 border-stone-200"/>'); continue; }
    ...
    // Checkbox list
    if (/^[\-\*]\s\[[ x]\]\s/.test(line)) {
      if (!inList || listType !== 'ul') { closeList(); out.push('<ul class="ml-2 my-3 space-y-1.5">'); inList = true; listType = 'ul'; }
      const checked = /\[x\]/i.test(line);
      const content = line.replace(/^[\-\*]\s\[[ x]\]\s*/, '');
      out.push(`<li class="flex items-start gap-2 leading-relaxed"><span class="mt-0.5 text-base">${checked ? '☑' : '☐'}</span><span>${inlineFmt(content)}</span></li>`);
      continue;
    }
    ...
    // Regular paragraph
    out.push(`<p class="text-stone-700 leading-relaxed my-2">${inlineFmt(line)}</p>`);
  }
